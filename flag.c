...
*/
void		ReshapeFlag (int width, int height)
{
...
*/
void		KeyboardFlag (unsigned char key, int x, int y)
{
...
*/
void		MouseFlag (int button, int state, int x, int y)
{
...
// int main(int narg, char ** args)
{
  ...
  if (texture_load ("alpha.jpg", alpha_texture, GL_ALPHA, 256) != 0 ||
      texture_load ("reflection.jpg", caustic_texture, GL_RGB, 256) != 0)
  {
      printf("Failed to load jpg files\n");
      return;
  }
  for (i = 0; i < 256 * 256; i++)
    {
  ...
  // glutReshapeFlag (&ReshapeFlag);
  // glutKeyboardFlag (&KeyboardFlag);
  // glutMouseFlag (&MouseFlag);
  // glutMotionFlag (&MotionFlag);
